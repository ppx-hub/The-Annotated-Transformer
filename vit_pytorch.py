...
class Attention(nn.Module):
    ...
    def forward(self, x):  # https://zhuanlan.zhihu.com/p/442125846
        ...
        # q, k, v都做维度变换，(batch, 197, 1024) -> (batch, 16, 197, 1024 / 16 = 64)
        # 16是head的数量，目的是做**多头**注意力机制
...
class ViT(nn.Module):
    ...
    def forward(self, img):     # https://blog.csdn.net/weixin_44966641/article/details/118733341
        ...
        x = torch.cat((cls_tokens, x), dim=1)   # 将cls_token拼接到patch token中去       (b, 197, 768)
        x += self.pos_embedding[:, :(n + 1)]    # 加位置嵌入（直接加）      (b, 197, 768)
        x = self.dropout(x)
